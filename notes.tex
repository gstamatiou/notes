...
\]
\item The only spacelike cocompact representation of $\SL(d,\mathbb R)$ with $p = \dim(G/K)$ is the symmetric cube
\[
\rho:\SL(2,\mathbb R) \to \Sym^3(\mathbb R^2) \to \SO(2,2).
\]
\begin{proof}
...
\[
\frac{1}{2} \cdot \frac{d!}{i!(d-2i)!} \geq \frac{1}{2} \cdot \frac{d!}{2!(d-4)!} > \frac{d(d+1)}{2} - 1, \text{ for } d \geq 4.
\]
...
\]
Hence it suffices to check the case $d = 2,3$, $i=1$ and $(i,d) = (1,2), (2,4)$.
The case $(i,d) = (2,4)$ corresponds to the isomorphism $\SL(4,\mathbb R) \to \SO(3,3)$, which is not spacelike cocompact.
The case $i = 1$ corresponds to the adjoint representation 
\[
\Ad: \SL(d,\mathbb R) \to \SO(\frac{d(d+1)}{2}-1, \frac{d(d-1)}{2}),
\]
...
Finally, for the case $d = 2$, we have that only the symmetric cube $\Sym^3(\mathbb R^2)$ which is in $\SO(2,2)$.
More concretely, we have that $\Sym^3(\mathbb R^2) \simeq \Sp(4, \mathbb R)$, but composing with the exceptional isomorphism $\Sp(4, \mathbb R) \to \SO(2,2)$, we obtain a representation $\rho: \SL(2,\mathbb R) \to \SO(2,2)$.
...
\section{2025-06-24}
\subsection{Volume entropy for complex hyperbolic spaces}
\begin{definition}
    The complex hyperbolic space $\mathbb H^n_\mathbb C$ is defined as
    \[
    \mathbb H^n_\mathbb C = \left\{ z \in \mathbb P(\mathbb C^{n,1}): \langle z, z \rangle < 0 \right\}, 
    \]
    where the inner product $\langle \cdot, \cdot \rangle$ is the standard Hermitian inner product on $\mathbb C^{n,1}$:
    \[
    \langle z, w \rangle = z_1 \overline{w_1} + z_2 \overline{w_2} + \cdots + z_n \overline{w_n} - z_{n+1} \overline{w_{n+1}}.
    \]
\end{definition}
The induced metric on $\mathbb H^n_\mathbb C$ is the Bergman metric, given by the relation
\[
\cosh^2(\frac{d(z,w)}{2}) = \frac{\langle z,w \rangle \langle w,z \rangle}{\langle z,z \rangle \langle w,w \rangle}.
\]
We can identify $\mathbb H^n_\mathbb C$ with the unit ball in $\mathbb C^{n,1}$:
\[
\mathbb H^n_\mathbb C \to \mathbb B^n_\mathbb C = \left\{ z \in \mathbb C^{n}: |z_1|^2 + \cdots + |z_n|^2 < 1 \right\}, \quad z = (z_1, \cdots, z_n, z_{n+1}) \mapsto (z_1/z_{n+1}, \cdots, z_n/z_{n+1}).
\]
In these coordinates, the ball of radius $R$ is given by the $z \in \mathbb C^n$ such that
\[
\frac{1}{\left((1-|z|^2)^2\right)(-1)} < \cosh^2(R/2) \text{ or equivalently } |z|^2 < 1 - \cosh^{-2}(R/2).
\]
and the volume element is given by
\[
\d \mathrm{vol} = \frac{c}{\left(1 - |z|^2\right)^{2n-1}} \d x_1 \wedge \d y_1 \wedge \cdots \wedge \d x_n \wedge \d y_n,
\]
where $c$ is a positive constant, which does not matter when computing the volume entropy.
Thus we calculate (up to a constant factor)
\begin{align*}
    \mathrm{Vol} B_R([0:\cdots:0:1]) &\simeq
    \int_{|z|^2 < 1 - \cosh^{-2}(R/2)} \frac{1}{\left(1 - |z|^2\right)^{2n-1}} \d x_1 \d y_1 \cdots \d x_n \d y_n\\
\end{align*}
Then the volume of a ball is given by
\[
\mathrm{Vol} B_R([0:\cdots:0:1]) = c \sinh^4(R/2)
\]
which exponentially behaves like $e^{2R}$.
So the volume entropy is given by
\[
\lim_{R \to \infty} \frac{\log \mathrm{Vol} B_R([0:\cdots:0:1])}{R} = 2.
\]
\printbibliography
\end{document}