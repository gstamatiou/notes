...
\end{proof}
\section{Examples of critical exponents}
The critical exponent of a subgroup $\Gamma$ of $\SL(2,\mathbb R)$ is not the same as its critical exponent, seen as a subgroup of $\SO_0(2,1)$.
\begin{proposition}
    Let $\Gamma$ be a discrete subgroup of $\SL(2,\mathbb R)$, and denote with $\rho: \SL(2,\mathbb R) \to \SO_0(2,1)$ the irreducible isomorphism.
    Then
    \[
    \delta_{1,2}(\Gamma) = 2 \delta(\rho(\Gamma)).
    \]
    In particular, the Cartan projections $\mu: \SL(2,\mathbb R) \to \mathfrak a^+$ and $\mu': \SO_0(2,1) \to \mathfrak a'^+$ of $\SL(2,\mathbb R)$ and $\SO_0(2,1)$ are related via
    \[
    \mu'(\rho(g)) = (2\mu_1(g), 0, - 2\mu_1(g)).
    \]
\end{proposition}
\begin{proof}
    The equality for the critical exponents follows from the equality of the Cartan projections, so it suffices to prove the latter.

    Recall that the symmetric square of the standard representation of $\SL(2,\mathbb R)$ is an isomorphism $\rho: \SL(2,\mathbb R) \to \SO_0(\mathrm{Sym}^2 \mathbb R^2, B)$, where $B= \mathrm{Sym}^2(\det)$ is a bilinear form of signature $(2,1)$.
    In particular, in the base $e_1^2, e_1 e_2, e_2^2$ of $\mathrm{Sym}^2 \mathbb R^2$ we have
    \[
    B = \begin{pmatrix}
        0 & 0 & 2\\
        0 & 1 & 0\\
    ...
    \end{pmatrix}.
    \]
    We can calculate that in the same basis for $\Sym^2 \mathbb R^2$ and for the standard basis of $\mathbb R^2$ we have
    \[
    \rho_*\left(
    \begin{pmatrix}
        a & b\\
    ...
    Writing the Cartan decomposition $g = k \exp(\mu(g)) l$ for $k,l \in \SO(2)$, we have that $\rho(g) = \rho(k) \exp(\rho_*(\mu(g))) \rho(l)$ is a Cartan decomposition of $\rho(g)$, since $\rho(\SO(2))$ is a maximal compact subgroup of $\SO_0(2,1)$.
    Thus $\mu'(\rho(g)) = (2\mu_1(g), 0, -2\mu_1(g))$.
\end{proof}

Two calculations of Anosov representations
\begin{proposition}
    Let $\Gamma \subseteq \SL(2,\mathbb R)$ be a uniform lattice, and consider the irreducible representation $\rho_{\text{irr}}: \SL(2,\mathbb R) \to \SL(3,\mathbb R)$ and the reducible representation $\rho_{\text{red}}: \SL(2,\mathbb R) \times \SL(2,\mathbb R) \to \SL(3, \mathbb R)$.
    Both restrictions on $\Gamma$ are projective Anosov, and we have
    \begin{align*}
        2\delta_{1,3}(\rho_{\text{irr}}(\Gamma)) &= \delta_{1,2}(\rho_{\text{irr}}(\Gamma)) = 1 ,\\
        \delta_{1,3}(\rho_{\text{red}}(\Gamma)) &= \frac{1}{2}\delta_{1,2}(\rho_{\text{red}}(\Gamma)) = 1.
    \end{align*}
\end{proposition}

Before the next example, we will need the following theorem:
\begin{theorem}[Borel density theorem]
    Let $G$ be a semisimple real algebraic group and $\Gamma$ be a lattice in $G$.
    Then $\Gamma$ is Zariski-dense in $G$.
\end{theorem}

\begin{proposition}
    Let $\Gamma$ be a lattice of $\SL(2,\mathbb R)$ and $u: \Gamma \to \mathbb R^2$.
    We define $\rho_u: \Gamma \to \SL(3,\mathbb R)$ as
    \[
    \rho_u(\gamma) = \begin{pmatrix}
        \gamma & u(\gamma)\\
        0 & 1
    \end{pmatrix}.
    \]
    Then
    \begin{enumerate}
        \item $\rho_u$ is a representation if and only if $u$ is a cocycle:
        \[
        u(\gamma \eta) = u(\gamma) + \gamma u(\eta).
        \]
        \item $\rho_u$ is projective Anosov.
        \item The limit sets in the projective space are independent of $u$ and are all equal to the projective line $\mathbb P(\mathbb R^2 \times \{0\})$.
        \item The dual limit set $\xi_u^*: \partial_\infty \Gamma \to \mathbb P((\mathbb R^3)^*)$ is a projective line if and only if $u$ is a cobordism.
    \end{enumerate}
\end{proposition}
\begin{proof}
    \begin{enumerate}
        \item This is a straightforward calculation.
        \item First note that $\rho_0$ is projective Anosov, so $\rho_{tu}$ is projective Anosov for $t$ small enough, by the fact that the set of Anosov representations is open in the representation variety.
        Moreover, $\rho_u$ is conjugate to $\rho_{tu}$.
        \item First note that $W = \mathbb R^2 \times \{0\}$ is $\rho_u(\Gamma)$-invariant.
        This implies that the limit set $\Lambda_{\rho_u(\Gamma)}$ is contained in $\mathbb P(W)$.
        Indeed, one reason is that the projective limit set can be defined as the smallest closed $\rho_u(\Gamma)$-invariant subset of $\mathbb P(\mathbb R^3)$.
        Alternatively, for any $w \in W$, we can find some $\gamma_- \in \partial_\infty \Gamma$ such that $w \not \in \xi_u^*(\gamma_-)$, because $\partial_\infty \Gamma$ is infinite and $\xi_u^*$ is injective.
        Then $\lim_{n \to \infty} \rho_u(\gamma^n) \mathbb R w = \xi_u(\gamma_+) \in \mathbb P(W)$, where $\gamma_+$ is the attracting fixed point of $\gamma$.
        
        To see the reverse inclusion, note first that since $\Gamma$ is a lattice of $\SL(2,\mathbb R)$, its limit set is the whole $\mathbb P(\mathbb R^2)$.
        It is thus reasonable to expect that $\Lambda_{\rho_u(\Gamma)} = \mathbb P(W)$, given that the action of $\rho_u(\Gamma)$ on $W$ is the same as the action of $\Gamma$ on $\mathbb R^2$.
        Indeed, for $\mathbb R(x,0) \in \mathbb P(W)$, we can find a sequence $\gamma_n \in \Gamma$ such that $\lim_{n \to \infty} \gamma_n \mathbb R x = \mathbb R x'$.
        Then $\lim_{n \to \infty} \rho_u(\gamma_n) \mathbb R (x,0) = \mathbb R(\gamma_n x, 0) = \mathbb R x'$, so $\mathbb P(W) \subseteq \Lambda_{\rho_u(\Gamma)}$.
        \item First note that $\rho_u$ is conjugated to $\rho_0$ if and only if $u$ is a cobordism, so if $u$ is a cobordism then $\xi_u^*$ is a projective line.
        If $\xi_u^*(\partial_\infty \Gamma)$ is a projective line $\mathbb P(W^*)$, then $W^*$ is $\rho_u(\Gamma)$-invariant, so the annihilator $W = (W^*)^\perp$ is also $\rho_u(\Gamma)$-invariant and $\dim W = 1$. 
        
        We will show that $\rho_u$ does not preserve any line if $u$ is not a cobordism, which will conclude the proof.
        Assuming the contrary, writing $(w, t) \in W$, for every $\gamma$ there exists $\lambda(\gamma)$ such that
        \[
        \rho_u(\gamma)(w,t) = (\gamma w + t u(\gamma), t) = (\lambda(\gamma)w, \lambda(\gamma) t).
        \]
        If $t = 0$, this implies that $\gamma w = \lambda(\gamma) w$, so $\mathbb R w$ is $\Gamma$-invariant.
        But $\Gamma \subseteq \SL(2,\mathbb R)$ is a lattice, so by the Borel density theorem it is Zariski-dense, and thus does not preserve any line.
        Thus $t \neq 0$. The calculation above shows that $\lambda(\gamma) = 1$ for every $\gamma$, so $u(\gamma) = w - \gamma w$, i.e.\ $u$ is a cobordism.
    \end{enumerate}
\end{proof}
...
\subsubsection{Coordinate representation for metric}
    ...
    \begin{proposition}[Geodesic polar coordinates]
        In geodesic polar coordinates for the hyperboloid model, the hyperbolic metric is given by
        \[
    ...
    \begin{proof}
        ...
        \begin{align*}
            ...
            v_1 \frac{\partial}{\partial r^1} + v_2 \frac{\partial}{\partial r^2} &\mapsto \left(\sinh(\|v\|) \frac{v}{\|v\|}, \cosh(\|v\|)\right),
        \end{align*}
    ...
    \end{proof}
    We do the same for the hyperbolic $n$-space:
    \begin{proposition}
        Consider $\mathbb H^n$ in the hyperboloid model and let $p = e_{n+1}$.
        Then in geodesic polar coordinates centered at $p$, the hyperbolic metric is given by
        \[
        g = \d r^2 + \sinh^2(r) g_{\mathbb S^{n-1}},
        \]
        where $g_{\mathbb S^{n-1}}$ is the round metric on the sphere $\mathbb S^{n-1}$.
    \end{proposition}
    \begin{proof}
        By \cref{prop:geodesics_hyperboloid_model_plane} we have that the exponential map at $p$ is given by
        \begin{align*}
            \exp_p: T_p \mathbb H^n &\to \mathbb H^n\\
            v_1 \frac{\partial}{\partial r^1} + \cdots + v_n \frac{\partial}{\partial r^n} &\mapsto \left(\sinh(\|v\|) \frac{v}{\|v\|}, \cosh(\|v\|)\right),
        \end{align*}
        where $\|v_1 \partial_{r^1} + \cdots + v_n \partial_{r^n}\| = \sqrt{(v_1)^2 + \cdots + (v_n)^2}$ is the usual euclidean norm.
        We consider the polar coordinates on $\mathbb R^n$:
        \begin{align*}
            \mathbb R_{>0} \times \mathbb S^{n-1} &\to \mathbb R^n - \{0\}\\
            (r, \theta) &\mapsto r \theta,
        \end{align*}
        and compose with the exponential map to obtain the chart of geodesic polar coordinates:
        \begin{align*}
            \phi: \mathbb R_{>0} \times \mathbb S^{n-1} &\to \mathbb H^n-\{p\}\\
            (r, u) &\mapsto \exp_p(r u) = \left(\sinh(r) u, \cosh(r)\right).
        \end{align*}
        Then we compute the frame given by the coordinate vector fields with respect to the ambient coordinates of $\mathbb R^{n,1}$:
        \begin{align*}
            \frac{\partial}{\partial r} &= \left( \cosh(r)u, \sinh(r) \right) = 
            \cosh(r) \sum_{i=1}^n u_i \frac{\partial}{\partial x^i} + \sinh(r) \frac{\partial}{\partial x^{n+1}}\\
            \frac{\partial}{\partial \theta_i} &= \sinh(r) \frac{\partial}{\partial \theta^i} + 0 \frac{\partial}{\partial x^{n+1}},
        \end{align*}
        where $\partial/\partial \theta^i$ are the coordinate vector fields of $\mathbb S^{n-1} \subseteq \mathbb R^n$.
        Then we compute the coefficients using the metric in the ambient coordinates given by the Minkowski metric of $\mathbb R^{n,1}$:
    \begin{align*}
        g_{rr} &= \langle \left(\cosh(r)u, \sinh(r)\right) \rangle_{n,1} = \cosh^2(r) \langle u, u \rangle + \sinh^2(r) = \cosh^2(r) + \sinh^2(r) = 1,\\
        g_{\theta_i \theta_j} &= \langle \sinh(r) \frac{\partial}{\partial \theta^i}, \sinh(r) \frac{\partial}{\partial \theta^j} \rangle_{n,1} =
        \langle \sinh(r) \frac{\partial}{\partial \theta^i}, \sinh(r) \frac{\partial}{\partial \theta^j} \rangle_{n,0} = \sinh^2(r) (g_{\mathbb S^{n-1}})_{ij},\\
        g_{r \theta_i} &= \langle \left(\cosh(r)u, \sinh(r)\right), \sinh(r) \frac{\partial}{\partial \theta^i} \rangle_{n,1} = 0,
    \end{align*}
    where the last equality holds since $u$ is orthogonal to $T_u \mathbb S^{n-1}$.
    \end{proof}
    Note that in the above coordinates, $r$ is the distance from the point $p$ and $\theta$ is the angle with respect to the $x^1$-axis, i.e.\ the geodesic starting at $p$ with initial velocity $r\left(\cos(\theta)\frac{\partial}{\partial x^1} + \sin(\theta)\frac{\partial}{\partial x^2}\right)$ is given by $t \mapsto \phi^{-1}(t, \theta)$.
\subsubsection{Geodesics}
\begin{proposition}[Geodesics in the hyperboloid model of $\mathbb H^2$]\label{prop:geodesics_hyperboloid_model_plane}
    Consider $\mathbb H^2$ in the hyperboloid model.
    Then the geodesic starting at $p = (0,0,1)$ with initial velocity $v = v_1 \partial_{r^1} + v_2 \partial_{r^2}$ is given by
    \[
    \gamma(t) = (\sinh(t\|v\|) \frac{v}{\|v\|}, \cosh(t\|v\|)).
    \]
    for $t \in \mathbb R$, where $\|v\| = \sqrt{v_1^2 + v_2^2}$ is the euclidean norm.
    Similarly, for $p = (0, \cdots, 0, 1) \in \mathbb H^n$ and $v = v_1 \partial_{r^1} + \cdots + v_n \partial_{r^n}$, the geodesic starting at $p$ with initial velocity $v$ is given by
    \[
    \gamma(t) = (\sinh(t\|v\|) \frac{v}{\|v\|}, \cosh(t\|v\|)).
    \]
    for $t \in \mathbb R$, where $\|v\| = \sqrt{v_1^2 + \cdots + v_n^2}$ is the euclidean norm.
\end{proposition} 
\begin{lemma}[Geodesics in embedded submanifolds]
    Let $(M,g)$ be a embedded submanifold of $\mathbb R^n$ with the metric induced by the Euclidean metric of $\mathbb R^n$.
    Then $\gamma: I \to M$ is a geodesic if and only if $\gamma''(t)$ is orthogonal to $T_{\gamma(t)} M$ for all $t \in I$.
\end{lemma}\todo{Show this for pseudoriemannian submanifolds}
\begin{proof}
    This is \cite[Corollary 5.2]{lee2018introduction}.
\end{proof}
\subsubsection{Volume entropy}
\begin{proposition}
    The volume entropy of the hyperbolic plane is equal to $1$.
\end{proposition}
\begin{proof}
    We use geodesic polar coordinates centered at a point $p = (0,0,1)$ in the hyperboloid model.
    Then the volume of a ball is given by
    \[
    \mathrm{Vol}(B(p,R)) = \int_{r < R} \sinh(r) \d r \d \theta = 2\pi (\cosh(R) - 1) \sim \pi e^R.
    \]
\end{proof}
We do the same for the hyperbolic $n$-space:
\begin{proposition}
    The volume entropy of the hyperbolic $n$-space is equal to $n-1$.
\end{proposition}
\begin{proof}
We use geodesic polar coordinates centered at a point $p = (0, \cdots, 0, 1)$ in the hyperboloid model.
    Then the volume of a ball is given by
    \[
    \mathrm{Vol}(B(p,R)) = \int_{r < R} \sinh^{n-1}(r) \d r \d \mathrm{Vol}_{\mathbb S^{n-1}} \sim \mathrm{Vol}(\mathbb S^{n-1}) \frac{e^{(n-1)R}}{2^{n-1}(n-1)}.
    \]
    Indeed, we have that $\sinh(r) \sim e^r/2$ for $r \to +\infty$, so $\sinh^{n-1}(r) \sim e^{(n-1)r}/2^{n-1}$.
    Thus, we obtain the result.

    If we want to be more precise, we can compute the limit:
    \begin{align*}
        \lim_{R \to +\infty} \frac{1}{R} \log(\mathrm{Vol}(B(p,R)))
    \end{align*}
...
\begin{definition}
    Let $G$ be a group and $\mu$ be a probability measure on $G$.
    Consider the space of sequences $\Omega = G^\mathbb N$ endowed with the product measure $\mathbb P = \mu^{\otimes \mathbb N}$.
    Then the \emph{random walk} on $G$ with independently $\mu$-distributed increments is the random variable $Z: \Omega \times \mathbb N \to G$ defined as
    \[
    Z_n(\omega) = g_1 g_2 \cdots g_n, \text{ where } \omega = (g_i)_{i \in \mathbb N} \in \Omega.
    \]
\end{definition}
Indeed, if we denote with $g_i: \Omega \to G$ the projection on the $i$-th coordinate, then we have that $g_i$ are independent and identically distributed with law $\mu$.
...
\begin{definition}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item The measure $\mu \in \mathrm{Prob}(G)$ is \emph{admissible} if the support of $\mu$ generates $G$ as a semigroup and there exists $n \in \mathbb N$ such that $\mu^{*n}$ is not singular to the Haar measure on $G$.
    \end{enumerate}
\end{definition}
\begin{definition}[Harmonic functions]
    Let $G$ be a group and $\mu$ be a probability measure on $G$.
    A function $f: G \to \mathbb R$ is \emph{$\mu$-harmonic} if for every $g \in G$ we have
    \[
    f(g) = \int_G f d(g_* \mu), \text{ for all } g \in G.
    \]
\end{definition}
\begin{lemma}[Characterization of stationary measures]
    Let $(G, \mu)$ be a group with a probability measure, $X$ be a space on which $G$ acts measurably and $\nu$ be a measure on $X$.
    We define $P_\nu: L^\infty(X, \nu) \to L^\infty(G, \mu)$ as
    \[
    P_\nu f(g) = \int_X f d(g_* \nu).
    \]
    Then $\nu$ is $\mu$-stationary if and only if $P_\nu$ maps $L^\infty(X, \nu)$ to the space of $\mu$-harmonic functions on $G$.
\end{lemma}
...
\begin{theorem}[Osedelec's pointwise multiplicative theorem]\label{thm:osedelec}
    ...
    \end{enumerate}
\end{theorem}
\subsection{Osedelec's theorem and equivalent definitions of Lyapunov exponents}\todo{What is the relationship between directed and nondirected exponents?}
...
\begin{enumerate}[label=(\roman*)]
    ...
    This follows from the Cartan decomposition of $g$.
...
\end{proof}
\begin{proposition}
    Let $g \in \SL(d, \mathbb R)$ and consider the cocycle $w: \mathbb Z \times \{*\} \to \SL(d, \mathbb R)$ defined by $w(n, *) = g^n$ (here the measurable space $X$ is a singleton).
    Then the Lyapunov spectrum of $w$ is related to the Jordan projection of $g$ as follows:
    \[
    \lambda(g) = (\chi_1, \chi_1 + \chi_2, \ldots, \chi_1 + \cdots + \chi_d) \in \mathfrak a^+,
    \]
    where $\chi_1, \ldots, \chi_d$ are the Lyapunov exponents of $w$.
\end{proposition}
\begin{proof}
Recall that for a matrix $g \in \SL(d,\mathbb R)$, the Lyapunov and the Cartan projection are related by 
\[
\lambda(g) = \lim_{n \to \infty} \frac{1}{n} \mu(g^n).
\]
But $\mu(g) = (\log \sigma_1(g), \ldots, \log \sigma_d(g))$, so the result follows from the third point of the definition of nondirected Lyapunov exponents.
\end{proof}
...
\subsection{Equivalent cocycles}
    ...
    \end{definition}
\subsection{Lyapunov exponents and random products}
The goal of this section is to motivate the study of gregs and their Lyapunov exponents, by showing how they arise naturally when studying random products of matrices.

Consider a discrete-time ergodic dynamical system $(X, \mathcal X, m, T)$ and a measurable map $\alpha: X \to \mathbb R$.
To understand this system, a fundamental question is to study the asymptotic behaviour of
\[
\alpha(T^{n-1}(x)) + \cdots + \alpha(x) \text{ as } n \to \infty.
\]
The answer comes from Birkhoff's ergodic theorem, which states that for $m$-almost every $x \in X$ we have
\[
\alpha(T^{n-1}(x)) + \cdots + \alpha(x) \sim n \int_X \alpha(x) dm(x).
\]

A natural generalization is to consider an ergodic dynamical system $(X, \mathcal X, m, T)$ and a measurable map $w: X \to \mathbb GL(d,\mathbb R)$, and study the asymptotic behaviour of the product
\[
w(T^{n-1}(x)) \cdots w(x) \text{ as } n \to \infty.
\]
Note that the product is exactly $w(n,x)$, where $w(n,x)$ is the cocycle generated by $w$, so our first observation is that we arrive at the study of gregs.
On the other hand, Osedelet's multiplicative ergodic theorem tells us that for $m$-almost every $x \in X$ there the Lyapunov exponents $\chi_1(x) > \cdots > \chi_k(x)$ and a decomposition $\mathbb R^d = E_1(x) \oplus \cdots \oplus E_k(x)$ such that for every $v \in E_i(x) \setminus \{0\}$ we have
\[
\|w(n,x) v\| \sim e^{n \chi_i(x)},
\]
while Kingman's subadditive ergodic theorem tells us that for $m$-almost every $x \in X$
\[
\log \|w(n,x)\| \sim e^{n \chi_1(x)} \int_X \log \|w(x)\| dm(x).
\]
...
\section{2024-10-07}
A reference for the discussion below is \cite{tholozan2019teichmuller}.
...
Consider a representation $\rho: \pi_1(S) \to \PSL(2, \mathbb R)$ and the associated hyperbolic metric on $S \simeq \rho(\pi_1(S)) \backslash \mathbb H^2$.
Then we can consider the geodesic flow $\phi_t: T^1 S \to T^1 S$ on the unit tangent bundle of $S$, construct a bundle $E_\rho = T^1 \mathbb H^2 \times \mathbb R^2 / \pi_1(S)$ over $T^1 S$ and lift the geodesic flow to a linear flow $\psi_t: E_\rho \to E_\rho$ on $E_\rho$, à la \cite{bochi2019anosov}.
In this way, the choice of a parametrization of the geodesic flow on $S$ (i.e.\ a choice of a hyperbolic metric) is compatible with the choice of a representation $\rho: \pi_1(S) \to \PSL(2, \mathbb R)$ used to construct the bundle $E_\rho$ and the linear flow $\psi_t: E_\rho \to E_\rho$.

However, one could consider a different parametrization of the geodesic flow on $S$ induced by a different hyperbolic metric.
It is a fact that the geodesic flows on $S$ induced by two different hyperbolic metrics are orbit equivalent, i.e.\ there exists a homeomorphism $h: T^1 S \to T^1 S$ that sends orbits of one flow to orbits of the other.
Using the same parametrization, we could then lift the geodesic flow to a linear flow on $E_\rho$.
The new flow $\psi_t': E_\rho \to E_\rho$ is dominated if and only if the original flow $\psi_t: E_\rho \to E_\rho$ is dominated, which makes sense given the fact that $\rho$ being Anosov does not depend on the choice of a hyperbolic metric on $S$.

Thus there is no reason to look for compatibility in the choice of a hyperbolic metric on $S$ and a representation $\rho: \pi_1(S) \to \PSL(2, \mathbb R)$.
More on that can be found in 

To see how the orbital equivalence of two geodesic flows on $S$ works, consider two hyperbolic metrics $g, g'$ on $S$ and the associated geodesic flows $\phi_t: T^1 S \to T^1 S$ and $\phi_t': T^1 S \to T^1 S$.
Let $j: \pi_1(S) \to \PSL(2, \mathbb R)$ and $j': \pi_1(S) \to \PSL(2, \mathbb R)$ be the two representations associated to $g$ and $g'$ respectively.
To recall how to construct these representations, we consider the universal cover $\tilde S$ of $S$.
Then one can lift the metrics $g$ and $g'$ to metrics on $\tilde S$, for which the fundamental group $\pi_1(S)$ acts by isometries.
Since $\tilde S$ with each of these metrics is a simply connected, complete, Riemannian manifold of constant curvature $-1$, we can obtain two isometries $f, f': \tilde S \to \mathbb H^2$.
Then we can define for $f(\tilde x) \in \mathbb H^2$ and $\gamma \in \pi_1(S)$:
\[
j(\gamma) \cdot f(\tilde x) = f(\gamma \cdot \tilde x),
\]
and similarly for $j'$.

In particular, the maps $f, f'$ are equivariant, so the map $f' \circ f^{-1}: \mathbb H^2 \to \mathbb H^2$ is a $j$-$j'$-equivariant isometry.
This isometry induces a homeomorphism $\partial_\infty (f' \circ f^{-1}): \partial_\infty \mathbb H^2 \to \partial_\infty \mathbb H^2$ that is also $j$-$j'$-equivariant.
We consider the $(j,j')$-equivariant homeomorphism $\tilde h: \partial_\infty \mathbb H^{2^{(3)}} \to \partial_\infty \mathbb H^{2^{(3)}}$.
Identifying $T^1 \mathbb H^2$ with $\partial_\infty \mathbb H^{2^{(3)}}$ using the orthogonal projection of the third point on the geodesic defined by the first two points, we obtain a $j$-$j'$-equivariant homeomorphism $\tilde h: T^1 \mathbb H^2 \to T^1 \mathbb H^2$ that descends to a homeomorphism $h: T^1 S \to T^1 S$.
One can check that $h$ sends orbits of $\phi_t$ to orbits of $\phi_t'$, giving us the desired orbit equivalence (see \cref{fig:orbit_equivalence}).
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/orbital_equivalence.png}
    \caption{Orbit equivalence between geodesic flows associated to different hyperbolic metrics on $S$.}
    \label{fig:orbit_equivalence}
\end{figure}

One should note that the homeomorphism $h: T^1 S \to T^1 S$ is not induced from a diffeomorphism of $S$.
Thus we have proved that 
\begin{proposition}
    All geodesic flows on a closed surface $S$ of genus $g \geq 2$ are orbit equivalent.
\end{proposition}

\section{2024-10-9}


\begin{theorem}[Equivalence of notions of entropy]
    Let $\pi_1(M)$ be the fundamental group of a closed negatively curved manifold $M$. Then the following notions of entropy coincide:
    \begin{itemize}
        \item the topological entropy of the geodesic flow on $T^1 M$;
        \item the volume entropy of $M$ (i.e.\ the exponential growth rate of the volume of balls in the universal cover of $M$);
        \item the critical exponent of $\pi_1(M)$ seen as a discrete subgroup of the isometry group of the universal cover of $M$:
        \[
        \delta(\pi_1(M)) = \limsup_{R \to \infty} \frac{1}{R} \log \#\{\gamma \in \pi_1(M) : d(\tilde x, \gamma \tilde x) \leq R\},
        \]
        where $\tilde x$ is a point in the universal cover of $M$.
        \item The critical exponent of the length spectrum of $M$:
        \[
        \delta_{\text{length}}(\pi_1(M)) = \limsup_{R \to \infty} \frac{1}{R} \log \#\{[\gamma] \in [\pi_1(M)] : \ell([\gamma]) \leq R\},
        \]
        where $[\pi_1(M)]$ is the set of conjugacy classes of $\pi_1(M)$ and $\ell([\gamma])$ is the length of the closed geodesic in the free homotopy class associated to $[\gamma]$.
    \end{itemize}  
\end{theorem}

\begin{lemma}
        Let $X$ be a connected topological space and $x_0 \in X$.
        Then the conjugacy classes of $\pi_1(X, x_0)$ are in bijection with the free homotopy classes of loops in $X$:
        \begin{align*}
            [\pi_1(X, x_0)] &\to \{\text{free homotopy classes of loops in } X\},\\
            [\gamma] &\mapsto \{\text{loops freely homotopic to } \gamma\}\\
            [\alpha^{-1} \gamma \alpha] &\mapsfrom [\gamma],
        \end{align*}
        where $\alpha$ is any path from $x_0$ to $\gamma(0)$.
    \end{lemma}
    \begin{proposition}
        Let $\Gamma$ be a uniform lattice of $\SO(n,1)$.
        Then $\Gamma \simeq \pi_1(\mathbb H^n / \Gamma)$, and in every conjugacy class of $\pi_1(\mathbb H^n / \Gamma)$, there exists a unique closed geodesic.
        In other words, for every closed loop on a compact hyperbolic manifold, there exists a unique closed geodesic in its free homotopy class.
    \end{proposition}
    The above is based on the following more general fact:
    \begin{proposition}
        Let $M$ be a complete compact Riemannian manifold.
        Then in every conjugacy class of $\pi_1(M)$, there exists a closed geodesic.
        In other words, for every closed loop on a compact manifold, there exists a closed geodesic in its free homotopy class.
    \end{proposition}

    \begin{proposition}
        Let $\Gamma$ be a uniform lattice of $\SO(n,1)$, and $M = \mathbb H^n/\Gamma$.
        Then $\Gamma$ is in bijection with the set of closed geodesics of $M$.
        All of its elements are hyperbolic, so they define attracting and repelling fixed points on $\partial_\infty \mathbb H^n$.
        For a conjugacy class $[\gamma]$ of $\Gamma$, $\gamma$ fixes a unique unit-speed geodesic $\tilde \alpha_\gamma$ in $\mathbb H^n$, which is the one with endpoints the attracting and repelling fixed points $\gamma^\pm \in \partial_\infty \mathbb H^n$ of $\gamma$.
        The geodesic in $M$ corresponding to $[\gamma]$ is the projection $\alpha_\gamma$ of this geodesic in $\mathbb H^n$.
        In particular, $\gamma$ acts on $\tilde \alpha_\gamma$ as a translation of length $\ell([\gamma]) = \inf_{x \in \mathbb H^n} d(x, \gamma x)$:
        \begin{align*}
            \gamma \cdot \tilde \alpha_\gamma(t) &= \tilde \alpha_\gamma(t + \ell([\gamma])), \quad t \in \mathbb R.\\
            \ell([\gamma]) &= d^{\mathbb H^n}(\tilde \alpha_\gamma(t), \gamma \tilde \alpha_\gamma(t)) = d^{M}(\alpha_\gamma(t), \alpha_\gamma(t + \ell([\gamma]))), \quad t \in \mathbb R,
        \end{align*}
        where $d^{\mathbb H^n}$ and $d^M$ are the distance functions on $\mathbb H^n$ and $M$ respectively.
        
        The length $\ell([\gamma])$ is given by
        \[
        \ell([\gamma]) = \mu_1(\gamma) = \frac{1}{2}\alpha_{1,n+1}(\mu(\gamma)) = \alpha_{1,2}(\mu(\gamma)),
        \]
        where $\mu: \SO(n,1) \to \mathfrak a^+$ is the Cartan projection.
    \end{proposition}

    \section{2024-10-10}
    Consider Labourie's property for the irreducible representation $\rho = \Sym^3\mathbb R^2: \SL(2,\mathbb R) \to \SL(4,\mathbb R)$.
    Then $\rho(\SL(2,\mathbb R)) \subseteq \Sp(4, \mathbb R)$.

    \begin{lemma}
        The group $\Sp(2n, \mathbb R)$ has Labourie's property, i.e.\ for every $V, W \in \mathrm{Gr}_n(\mathbb R^{2n})$, there exists some $g \in \Sp(2n, \mathbb R)$ such that $g V \cap W = 0$.
    \end{lemma}
    \begin{proof}
        We do the proof for $n=2$, and assume the contrary for some $V, W \in \mathrm{Gr}_2(\mathbb R^4)$.

        If $V$ is totally isotropic, then by Witt's theorem, there exists $g_0 \in \Sp(4, \mathbb R)$ such that $g_0 V = V_0 = \mathbb R e_1 \oplus \mathbb R e_2$.
        If not, then there exists $g \in \Sp(4, \mathbb R)$ such that $g_1 V = V_1 = \mathbb R e_1 \oplus \mathbb R e_3$.
        In both cases, we have $g V \cap W \neq 0$ for all $g \in \Sp(4, \mathbb R)$ if and only if $g V_0 \cap W = g g_i V \cap W \neq 0$ for all $g \in \Sp(4, \mathbb R)$.
        Similarly, we can assume that $W$ is either $W_0 = \mathbb R e_3 \oplus \mathbb R e_4$ or $W_1 = \mathbb R e_2 \oplus \mathbb R e_4$.

        Thus, if $V$ and $W$ are both totally isotropic, then we may assume that $V = \mathbb R e_1 \oplus \mathbb R e_2$ and $W = \mathbb R e_3 \oplus \mathbb R e_4$.
        
        If $V$ is totally isotropic and $W$ is not, then we may assume that $V = \mathbb R (e_1 + e_4) \oplus \mathbb R e_2$ and $W = \mathbb R e_1 \oplus \mathbb R e_3$.

        If $V$ and $W$ are both not totally isotropic, then we may assume that $V = \mathbb R e_1 \oplus \mathbb R e_3$ and $W = \mathbb R e_2 \oplus \mathbb R e_4$.
    \end{proof}
    ...
    \end{pmatrix}
    \]
    in the basis $\{e_1^3, e_1^2 e_2, e_1 e_2^2, e_2^3\}$ of $\Sym^3 \mathbb R^2$.
    Also I think that 
    \begin{align*}
        \xi: &\SL(2,\mathbb R)/P \to \mathcal F(\mathrm{Sym}^3 \mathbb R^4)\\
    \xi(g P) &= \rho(g) \cdot (e_1^3 \subseteq e_1^3 \oplus e_1^2 e_2 \subseteq e_1^3 \oplus e_1^2 e_2 \oplus e_1 e_2^2 \subseteq e_1^3 \oplus e_1^2 e_2 \oplus e_1 e_2^2 \oplus e_2^3)
    \end{align*}
    
    One idea is to assume that $\rho(\Gamma)$ is $P_2$-Anosov and look at the relation $\rho(g) V \cap W = 0$ in the projective space $\mathbb P(\mathbb R^4)$.
    I think that when $\Gamma \leq \SL(2,\mathbb R)$ is a uniform lattice, then $\rho(\Gamma)$ is $P_2$-Anosov.
    If we assume that $V = \xi^2(x)$ then Labourie's property holds for every $W \in \xi^2(\partial_\infty \Gamma)$ by transversality.
    If $W \notin \xi^2(\partial_\infty \Gamma)$, then Labourie's property would not hold only if every point of the set $\xi^2(\partial_\infty \Gamma)$ of projective lines intersects the projective line $\mathbb P(W)$.

\section{2024-10-13 - Notes on \cite{glorieux_hausdorff_2023}}
\subsubsection{G-invariant distances in rank 1 symmetric spaces}
Recall that rank $1$ symmetric spaces are irreducible, meaning that $\mathfrak p$ does not decompose as a direct sum of two non-trivial $\Ad(K)$-invariant subspaces, or equivalently that $\Ad(K)$ acts irreducibly on $\mathfrak p$.
This is a consequence of the classification of symmetric spaces, since the rank is additive when taking products.
\begin{proposition}[Uniqueness of invariant metrics on rank 1 symmetric spaces]
    Let $G$ be a semisimple Lie group and $K$ be a maximal compact subgroup of $G$.
    If $G$ is of rank $1$, then it admits a unique (up to scaling) $G$-invariant Riemannian metric that descends to a Riemannian metric on $G/K$.
    Here $G$ is acting on itself and $G/K$ by left multiplication.
\end{proposition}
\begin{proof}
    Suppose that $G$ admits a metric $\tilde Q$ that descends to a Riemannian metric $Q$ on $G/K$ that is $G$-invariant.
    Then $(G, \tilde Q) \to (G/K, Q)$ is an isometry.

    We construct a metric $\tilde Q$ on $\mathfrak g$ that is $\Ad(K)$-invariant.
    Since for $k \in K$, the following diagram commutes:
    \begin{center}
    \begin{tikzcd}[column sep=large, row sep=large]
        G \arrow[r, "\mathrm{Ad}_k"] \arrow[d] & G \arrow[d] \\
        G/K \arrow[r, "L_k"] & G/K
    \end{tikzcd}    
    \end{center}
    we differentiate at the identity and restrict to $\mathfrak p = \mathfrak k^\perp $ with respect to the Killing form to obtain the commutative diagram
    \begin{center}
    \begin{tikzcd}[column sep=large, row sep=large]
        \mathfrak g \arrow[r, "\mathrm{Ad}_k"] \arrow[d, "d\pi_e"] & \mathfrak g \arrow[d, "d\pi_e"] \\
        T_{o}(G/K) \arrow[r, "dL_k"] & T_{o}(G/K)
    \end{tikzcd}    
    \end{center}
    where $o = eK \in G/K$.
    Defining $\tilde Q$ as the pullback of $Q$ via $d\pi_e: \mathfrak p \to T_o(G/K)$, the upper horizontal arrow is an isometry since all the other arrows are isometries.

    But for rank $1$ groups, $\Ad(K)$ acts irreducibly on $\mathfrak p$, so by Schur's lemma, $\tilde Q$ is unique up to scaling.
    Since it lifts $Q$, the metric $Q$ is also unique up to scaling.
\end{proof}
The idea behind the above statement is that in rank $1$, there is a unique way to measure distances in $G/K$ that is compatible with the action of $G$.

\subsubsection[Importance of critical exponents]{Importance of critical exponents $\delta_{1,2}$, $\delta_{1,n}$}
The material here is a comment on the beginning of \cite[Section 1.2]{glorieux_hausdorff_2023}.

Let $\Gamma$ be a projective Anosov subgroup of $\SL(n,\mathbb R)$.
Then we can consider the critical exponents $\delta_{1,2}(\Gamma)$ and $\delta_{1,n}(\Gamma)$, which are the exponential growth rates of the number of elements $\gamma \in \Gamma$ such that $\alpha_{1,2}(\mu(\gamma)) \leq R$ and $\alpha_{1,n}(\mu(\gamma)) \leq R$ respectively.

The first is important be cause it measures the Anosovness of $\Gamma$, since $\Gamma$ is projective Anosov if and only if $\alpha_{1,2}(\mu(\gamma))$ grows at least linearly with $\|\mu(\gamma)\|$.
So if $\alpha_{1,2}(\mu(\gamma))$ grows quickly, then $\Gamma$ is "very Anosov" and $\delta_{1,2}(\Gamma)$ is large.

The second is important because it is equal to the critical of $\Gamma$ associated with the Hilbert metric on the symmetric space $\SL(n, \mathbb R)/\SO(n)$.
Indeed, the quantity $\mu(g \cdot o, h \cdot o) = \mu(g^{-1} h) \in \mathfrak a$ for $g, h \in \SL(n, \mathbb R)$ and $o = e\SO(n) \in \SL(n, \mathbb R)/\SO(n)$, is a  "vector valued distance" (see \cite[Section 2.2.2]{glorieux_hausdorff_2023}).
But we can identify $\SL(d,\mathbb R)/\SO(d)$ with the convex set $\Omega$ of positive definite matrices of determinant $1$ in $\mathbb P(\Sym^2 (\mathbb R^d)^*)$.
Then the Hilbert distance $d_\Omega$ on $\Omega$ is given by
\[
d_\Omega(g \cdot o, h \cdot o) = \alpha_{1,2}(\mu(g\cdot o, h \cdot o)).
\]
Thus $d(o, \gamma \cdot o) = \alpha_{1,n}(\mu(\gamma))$ for $\gamma \in \SL(n, \mathbb R)$, so
\[
\delta_{1,n}(\Gamma) = \limsup_{R \to \infty} \frac{1}{R} \log \#\{\gamma \in \Gamma : d_\Omega(o, \gamma \cdot o) \leq R\}.
\]

\subsubsection{Reduction of projective Anosov to strongly projectively convex cocompact}
Let $\Gamma$ be a projective Anosov subgroup of $\SL(n, \mathbb R)$.
The goal is to show that (\cite[Corollary 1.2]{glorieux_hausdorff_2023})
\[
\delta_{1,n}(\Gamma) \leq \dim_H(\xi^{\mathrm{sym}}(\partial_\infty \Gamma)) \leq \delta_{1,2}(\Gamma)
\]
assuming that for $\Gamma$ strongly projectively convex cocompact subgroup of $\SL(n, \mathbb R)$, we have (\cite[Theorem 1.1]{glorieux_hausdorff_2023})
\[
2\delta_{1,n}(\Gamma) \leq \dim_H(\xi^{\mathrm{sym}}(\partial_\infty \Gamma)) \leq \delta_{1,2}(\Gamma).
\]

It is based on the following:
\begin{lemma}
    Let $I: \SL(n, \mathbb R) \to \SL(\Sym^2 (\mathbb R^n)^*)$ be the representation induced by the action of $\SL(n, \mathbb R)$ on the space of symmetric bilinear forms on $\mathbb R^n$.
    Then for $g \in \SL(n, \mathbb R)$, we have
    \begin{align*}
        \mu(I(g)) &= (-2\mu_n(g), -\mu_{n-1}(g) - \mu_n(g), \ldots, -\mu_1(g) - \mu_2(g), -2\mu_1(g))\\
        \alpha_{1,2}(\mu(I(g))) &= \alpha_{n-1,n}(\mu(g)) = \alpha_{1,2}(\mu(g^{-1})),\\
        \alpha_{1,n}(\mu(I(g))) &= 2\alpha_{1,n}(\mu(g)).
    \end{align*}
\end{lemma}
\begin{proof}
    One can calulate that $I_*(\mu(g)) e^i e^j = - \left( \mu_i(g) + \mu_j(g) \right) e^i e^j$ for $1 \leq i \leq j \leq n$, where $\{e^1, \ldots, e^n\}$ is the dual basis of the canonical basis of $\mathbb R^n$.
    Thus $\mu(I(g))$ is as in the statement and the formulas for $\alpha_{1,2}(\mu(I(g)))$ and $\alpha_{1,n}(\mu(I(g)))$ follow.
\end{proof}

\subsubsection{A conjecture}
\begin{definition}
    Let $\Gamma$ be an infinite discrete subgroup of $\PGL(n, \mathbb R)$.
    We say that $\Gamma$ is \emph{strongly projectively convex cocompact} if it preserves a strictly and properly convex open set $\Omega \subseteq \mathbb P(\mathbb R^n)$ whose boundary is $C^1$ and acts cocompactly on the convex hull of its orbital limit set in $\Omega$.
\end{definition}
Recall that if $\Gamma$ is projective Anosov and preserves a properly convex open set $\Omega \subseteq \mathbb P(\mathbb R^n)$, then it is strongly projectively convex cocompact.
\begin{conjecture}
    Let $\Gamma$ be a strongly projectively convex cocompact subgroup of $\SL(n, \mathbb R)$.
    Then $2\delta_{1,n}(\Gamma) = \dim_H(\xi^{\mathrm{sym}}(\partial_\infty \Gamma))$ if and only if $\Gamma$ is conjugate to a subgroup of $\SO(n-1,1)$.
\end{conjecture}
If we had for instance that $\Gamma$ acts cocompactly on the whole strictly and properly convex open set $\Omega$, then this follows from the fact that in that case $\dim_H(\xi^{\mathrm{sym}}(\partial_\infty \Gamma)) = n-1$ and Crampon's result:
\begin{proposition}[Crampon]
    Let $\Gamma$ be a Gromov hyperbolic group acting properly discontinuously and cocompactly on a strictly convex open set $\Omega \subseteq \mathbb P(\mathbb R^n)$ with $C^1$ boundary.
    Then $2 \delta_{1,n}(\Gamma) \leq n-1$, with equality if and only if $\Gamma$ is conjugate to a subgroup of $\SO(n-1,1)$.
\end{proposition}
On the other hand, the theorem has been proven for Hitchin representations of surface groups by Potrie and Sambarino.

\section{2024-10-14}
\begin{definition}
    \begin{enumerate}[label=(\roman*)]
        \item A \emph{Borel space} $(X, \mathcal X)$ is a set $X$ equipped with a $\sigma$-algebra $\mathcal X$.
        \item Two Borel spaces $(X, \mathcal X)$ and $(Y, \mathcal Y)$ are \emph{isomorphic} if there exist measurable maps $f: X \to Y$ and $g: Y \to X$ such that $f \circ g = \mathrm{id}_Y$ and $g \circ f = \mathrm{id}_X$. Equivalently, there exists a bijection $f: X \to Y$ such that both $f$ and $f^{-1}$ are measurable, i.e.\ $f^{-1}(\mathcal Y) = \mathcal X$.
        \item A \emph{standard Borel space} is a Borel space that is isomorphic to a Polish space (i.e.\ a separable completely metrizable topological space) equipped with its Borel $\sigma$-algebra.
        \item A \emph{Lebesgue space} $(X, \mathcal X, [m])$ is a standard Borel space $(X, \mathcal X)$ equipped with measure class $[m]$.
    \end{enumerate}
\end{definition}
We now define the notion of a measurable quotient. The reason that we use it is that for a Borel space $(X, \mathcal X)$ and an equivalence relation $\sim$ on $X$, the quotient $X/\sim$ does not have a good measure-theoretic structure in general.
So we take quotients with respect to a sub-$\sigma$-algebra of $\mathcal X$ instead.
\begin{definition}
    Let $(X, \mathcal X, [m])$ be a standard Borel space and $\mathcal A \subseteq \mathcal X$ be a sub-$\sigma$-algebra.
    Then the \emph{measurable quotient} of $(X, \mathcal X, [m])$ by $\mathcal A$ is the standard Borel space $(Y, \mathcal Y, [\nu])$ such that there exists a measurable map $\pi: X \to Y$ satisfying $\pi^{-1}(\mathcal Y) = \mathcal A$ and such that $[\pi_* m] = [\nu]$.

    It exists and it is unique up to isomorphism.
\end{definition}
In the definition above, $\mathcal A$ stands for the information that we keep from $X$ when taking the quotient.

\begin{example}
    Let $(X, \mathcal X, [m])$ be a standard Borel space and $T: X \to X$ be a measurable map that preserves the measure class $[m]$.
    Then we can consider the sub-$\sigma$-algebra 
    \[
    \mathcal A = \{ E \in \mathcal X : T^{-1}(E) = E \} 
    \]
    of $T$-invariant measurable subsets of $X$.
    The quotient of $(X, \mathcal X, [m])$ by $\mathcal A$ is called the \emph{ergodic decomposition} of $(X, \mathcal X, [m])$ with respect to $T$ and each fiber $\pi^{-1}(y)$ is a $T$-invariant ergodic component of $(X, \mathcal X, [m])$.
\end{example}

\subsubsection{Metric ergodicity}
\begin{definition}[Metric ergodicity]
    Let $\Gamma$ be a group acting on a standard probability space $(X, \mathcal X, [m])$ by measure class preserving transformations.
    We say that the action $\Gamma \curvearrowright (X, \mathcal X, [m])$ is \emph{metrically ergodic} if for every action of $\Gamma$ by isometries on a separable metric space $W$, the only $\Gamma$-equivariant measurable maps $f: X \to W$ are essentially constant:
    \[
    \mathrm{Map}_\Gamma(X, W) = W^\Gamma.
    \]
\end{definition}

Recall that a mean on a set $X$ is a linear functional $m: \ell^\infty(X) \to \mathbb R$ such that
\begin{itemize}
    \item $m(f) \geq 0$ for every $f \geq 0$;
    \item $m(1) = 1$.
\end{itemize}
\begin{definition}
    Let $\Gamma$ be a group acting on a set $X$.
    We say that $\Gamma \curvearrowright X$ is an \emph{Zimmer amenable action} if there exists a $\Gamma$-invariant mean on the space of bounded measurable functions on $X$.
\end{definition}

\subsection{More boundary theory}
\begin{lemma}[Existence of limit measures for $(G, \mu)$-spaces]
    Let $(G, \mu)$ be a locally compact group equipped with a probability measure, $(X, \nu)$ be a metrizable compact $(G, \mu)$-space.
    Denote by $Z_n : \Omega \to G$ the $n$-th step of the random walk associated with $\mu$.
    Then there exists a measurable map 
    \[
    \nu_\cdot : \Omega \to \mathrm{Prob}(X)
    \]
    such that for $\mathbb P$-almost every $\omega \in \Omega$, we have the weak convergence
    \[
    Z_n(\omega)_* \nu \to \nu_\omega.
    \]
    Moreover, $\nu$ is the average of the $\nu_\omega$'s:
    \[
    \nu = \int_\Omega \nu_\omega d\mathbb P(\omega).
    \]
\end{lemma}
\begin{lemma}[Limits of harmonic functions along random walks]
    Let $(G, \mu)$ be a locally compact group equipped with an admissible probability measure.
    Then for every harmonic function $f \in \mathcal H^\infty(G, \mu)$, the limits
    \[
    \tilde f(g, \omega) = \lim_{n \to \infty} f(g Z_n(\omega))
    \]
    exist for all $g \in G$ and $\mathbb P$-almost every $\omega \in \Omega$, where $Z_n$ is the $n$-th step of the random walk associated with $\mu$.
    We shall denote with $\tilde f : \Omega \to \mathbb R$ the function $\tilde f(e, \cdot)$.
\end{lemma}
\begin{definition}[$(G, \mu)$-boundary]
    Let $(G, \mu)$ be a locally compact group equipped with a probability measure.
    A \emph{$(G, \mu)$-boundary} is a $(G, \mu)$-space $(X, \nu)$ such that for all $\phi, \psi \in L^\infty(X, \nu)$, the transform $P_\nu: L^\infty(X, \nu) \to \mathcal H^\infty(G, \mu)$ satisfies
    \[
    \tilde P_\nu(\phi \psi) = \tilde P_\nu(\phi) \tilde P_\nu(\psi),
    \]
    In that case, we define the \emph{boundary map} $\mathrm{bnd}: (\Omega, \mathbb P) \to (X, \nu)$ by the realtion
    \[
    \tilde P_\nu(\phi)(\omega) = \phi(\mathrm{bnd}(\omega))
    \]
    for $\mathbb P$-almost every $\omega \in \Omega$ and all $\phi \in L^\infty(X, \nu)$.
\end{definition}
\begin{proposition}[Compact $(G, \mu)$-boundaries]
    If $(X, \nu)$ is a compact $(G, \mu)$-space, then it is a $(G, \mu)$-boundary if and only if there exists a measurable map $\mathrm{bnd}: \Omega \to X$ satisfying
    \[
    \nu_\omega = \lim_{n \to \infty} Z_n(\omega)_* \nu = \delta_{\mathrm{bnd}(\omega)}
    \]
    for $\mathbb P$-almost every $\omega \in \Omega$, where $Z_n$ is the $n$-th step of the random walk associated with~$\mu$.
\end{proposition}
\begin{proof}
    For a compact $(G, \mu)$-space $(X, \nu)$, one has that 
    \[
    \tilde P_\nu(\phi)(g, \omega) = \int_X \phi d g_*\nu_\omega
    \]
    This, along with the identity satisfied by $\tilde P_\nu$ for $(G, \mu)$-boundaries implies that the variance of $\phi$ with respect to $\nu_\omega$ is zero for $\mathbb P$-almost every $\omega \in \Omega$ and all $\phi \in L^\infty(X, \nu)$.
    Thus $\nu_\omega$ is a Dirac mass.
\end{proof}
\begin{remark}
    If $(X, \nu)$ is a $(G, \mu)$-boundary, then the transformation $P_\nu: L^\infty(X, \nu) \to \mathcal H^\infty(G, \mu)$ is an isometric embedding.
\end{remark}
The universal $(G, \mu)$-boundary is called the Poisson boundary of $(G, \mu)$:
\begin{theorem}[Poisson boundary]
    Let $(G, \mu)$ be a locally compact group equipped with an admissible probability measure.
    Then there exists a unique (up to isomorphism) $(G, \mu)$-boundary $(B, \nu)$, called the \emph{Poisson boundary} of $(G, \mu)$, characterized by each of the following properties:
    \begin{enumerate}[label=(\roman*)]
        \item Poisson representation: The map $P_\nu: L^\infty(B, \nu) \to \mathcal H^\infty(G, \mu)$ is an isometric isomorphism.
        In particular every bounded harmonic function on $(G, \mu)$ admits a unique $\phi \in L^\infty(B, \nu)$ such that $f = P_\nu(\phi)$ and which can be recovered from $f$ by the boundary map $\mathrm{bnd}: \Omega \to B$:
        \[
        \phi(\mathrm{bnd}(\omega)) = \tilde f(\omega) = \lim_{n \to \infty} f(Z_n(\omega)).
        \]
        \item Universality: The Poisson boundary $(B, \nu)$ is maximal in the sense that every $(G, \mu)$-boundary $(X, \nu_X)$ is a $G$-equivariant measurable quotient of $(B, \nu)$, i.e.\ there exists a measurable $G$-equivariant map $\pi: B \to X$ such that $\pi_* \nu = \nu_X$.
    \end{enumerate}
\end{theorem}
\section{2024-10-15}
\begin{proposition}
    Let $\rho: \Gamma \to \SO(p,q)$ be a projective Anosov representation.
    Denoting with $\xi^{\mathrm{sym}} = (\xi, \xi^*): \partial_\infty \Gamma \to \mathbb P(\mathbb R^d) \times \mathbb P((\mathbb R^d)^*)$ its symmetric limit map:
    \[
    \dim_\mathcal H \xi(\partial_\infty \Gamma) =
    \dim_\mathcal H \xi^*(\partial_\infty \Gamma) =
    \dim_\mathcal H \xi^{\mathrm{sym}}(\partial_\infty \Gamma).
    \]
\end{proposition}
\begin{proof}
    We assume that $\mathbb P(\mathbb R^d), \mathbb P((\mathbb R^d)^*)$ are equipped with the round metrics and the product with the $\infty$-product metric.
    This is convenient since for this kind of metric, we have that $\diam(U \times V) = \max\{\diam(U), \diam(V)\}$, which makes it easy to compute Hausdorff dimensions.

    Defining $f: \mathbb P(\mathbb R^d) \to \mathbb P((\mathbb R^d)^*)$ by $f(\mathbb R v) = \mathbb R v^\perp$, where $v^\perp$ is with respect to the preserved product, it is a smooth diffeomorphism between compact manifolds, so it is bi-Lipschitz, and thus preserves the Hausdorff dimension.

    But $\xi^* = f \circ \xi$ (\cite[Theorem 2.53]{glorieux_hausdorff_2023}), so $\xi^{\mathrm{sym}}(\partial_\infty \Gamma) = \mathrm{Gr}(f)$.
    Thus it suffices to show that in the product metric, the Hausdorff dimension of the graph of a bi-Lipschitz function is equal to the Hausdorff dimension of its domain.
    Indeed, for a cover $U_\alpha$ of the domain, $U_\alpha \times f(U_\alpha)$ is a cover of the graph and there exist constants $C_1, C_2 > 0$ such that
    \[
    C_1 \diam(U_\alpha) \leq \diam(U_\alpha \times f(U_\alpha)) \leq C_2 \diam(U_\alpha).
    \]
\end{proof}
\begin{proposition}
    Let $\rho_{\mathrm{red}}: \SL(d,\mathbb R) \to \SL(n,\mathbb R)$, be the reducible representation, where $n > d$, and define 
    \begin{align*}
        \xi_{\mathrm{red}}: \mathbb P(\mathbb R^d) &\to \mathbb P(\mathbb R^n)\\
        \mathbb R v &\mapsto \mathbb R (v, 0)\\
        \xi_{\mathrm{red}}^*: \mathbb P((\mathbb R^d)^*) &\to \mathbb P((\mathbb R^n)^*)\\
        \mathbb R \varphi &\mapsto \mathbb R \tilde \varphi\\
        \ker(\varphi) &\mapsto \ker(\varphi) \oplus \mathbb R^{n-d},
    \end{align*}
    where $\tilde \varphi(v, w) = \varphi(v)$ for $v \in \mathbb R^d$ and $w \in \mathbb R^{n-d}$.
    Then for any projective Anosov representation $\rho: \Gamma \to \SL(d, \mathbb R)$, the representation $\rho_{\mathrm{red}} \circ \rho: \Gamma \to \SL(n, \mathbb R)$ is projective Anosov with symmetric limit map $\xi_{\mathrm{red}}^{\mathrm{sym}} = (\xi_{\mathrm{red}} \circ \xi, \xi_{\mathrm{red}}^* \circ \xi^*)$ and
    \[
    \dim_\mathcal H \xi_{\mathrm{red}}^{\mathrm{sym}}(\partial_\infty \Gamma) = \dim_\mathcal H \xi^{\mathrm{sym}}(\partial_\infty \Gamma).
    \]
\end{proposition}
\begin{proof}
    The fact that this is the limit map follows from the fact that the limit map is the unique $\rho$-equivariant continuous map from $\partial_\infty \Gamma$ to the appropriate flag space (Stage, Lemma 1.4.4).
    The equality of the Hausdorff dimensions follows from the fact that $\xi_{\mathrm{red}}$ and $\xi_{\mathrm{red}}^*$ are smooth embeddings, so they are bi-Lipschitz onto their images.
\end{proof}
\end{document}