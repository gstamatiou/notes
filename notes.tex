...
\begin{proof}
    \begin{enumerate}
        ...
        If $\xi_u^*$ is a projective line $\mathbb P(W^*)$, then $W^*$ is $\rho_u(\Gamma)$-invariant, so the annihilator $W = (W^*)^\perp$ is also $\rho_u(\Gamma)$-invariant and $\dim W = 1$. 
...
\begin{lemma}
    ...
    \xi(g P) = \rho(g) \cdot (e_1^3 \subseteq e_1^3 \wedge e_1^2 e_2 \subseteq e_1^3 \wedge e_1^2 e_2 \wedge e_1 e_2^2 \subseteq e_1^3 \wedge e_1^2 e_2 \wedge e_1 e_2^2 \wedge e_2^3)
    \] 
...
\end{lemma}
\begin{lemma}
    For every harmonic function $f \in \mathcal H^\infty(G, \mu)$, the limits
    \[
...
\begin{definition}
    ...
    A \emph{$(G, \mu)$-space} is a $(G, \mu)$-space $(X, \nu)$ such that for all $\phi, \psi \in L^\infty(X, \nu)$, the transform $P_\nu: L^\infty(X, \nu) \to L^\infty(G, \mu)$ satisfies
    \[
    \tilde P_\nu(\phi \psi) = \tilde P_\nu(\phi) \tilde P_\nu(\psi),
    \]
\end{definition}
\begin{proposition}
    If $(X, \nu)$ is a compact $(G, \mu)$-boundary, then there exists a measurable map $\mathrm{bnd}: \Omega \to X$ satisfying
    \[
